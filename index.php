...
$stmt = $dbh->query('CREATE TABLE IF NOT EXISTS "carriers" ("idCarrier" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE, "carrier", "emailSMS")');
$stmt = $dbh->query('CREATE TABLE IF NOT EXISTS "groups" ("idGroup" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE, "groupName" NOT NULL)');
$stmt = $dbh->query('CREATE TABLE IF NOT EXISTS "send" ("idSend" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE, "idCarrierInSend" INTEGER, "idGroupInSend" INTEGER, "idUserInSend" INTEGER, "address")');
$stmt = $dbh->query('CREATE TABLE IF NOT EXISTS "usersRecipients" ("idUser" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE, "user", "pass", "fullName", "userStatus" INTEGER, "phone", "idCarrierInUser" INTEGER, "email")');
$dbh->commit();
